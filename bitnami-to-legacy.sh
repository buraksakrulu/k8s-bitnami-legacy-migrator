#!/usr/bin/env bash
# bitnami_to_legacy.sh
# Deployment/DaemonSet/StatefulSet/CronJob şablonlarında
#   .../bitnami/...  ->  .../bitnamilegacy/...
# dönüşümü yapar. plan/apply/verify/interactive/continue modları vardır.
...
NAMESPACE_SELECTOR="${NAMESPACE_SELECTOR:-}"  # ör: "kube-system,redis-kuhub"
KINDS="${KINDS:-deploy,ds,sts,cronjob}"

usage() {
  cat <<EOF
Kullanım:
  $0 [plan|apply|verify|continue|interactive]

ENV:
  STATE_FILE=bitnami-migration-state.jsonl
  TIMEOUT=180s
  NAMESPACE_SELECTOR="ns1,ns2"
...
# --- /dev/tty üzerinden okumayı garanti et ---
TTY_FD=
if [[ -r /dev/tty ]]; then
  exec 3</dev/tty
  TTY_FD=3
elif [[ -t 0 ]]; then
  # stdin terminal ise onu kullan
  TTY_FD=0
else
  TTY_FD=
fi
...
prompt_read() {
  # prompt_read <varname> <prompt_message>
  local __var="$1"; shift
  local __msg="${*:-> }"
  if [[ -n "${TTY_FD}" ]]; then
    ...
    echo "HATA: Etkileşimli girdi için TTY bulunamadı." 1>&2
    exit 2
  fi
}

# Kaynakları (deploy,ds,sts,cronjob) JSON olarak al
fetch_workloads() {
  local kinds="${KINDS}"
...
# ---------- jq yardımcıları ----------
# "bitnami/" → "bitnamilegacy/" (case-insensitive)
...
verify_rollout() {
  ...
  case "${kind}" in
    Deployment|DaemonSet|StatefulSet)
      ...
      ;;
    CronJob)
      echo "CronJob/${name}: şablon güncellendi (rollout yok)."
      ;;
  esac
}
...
write_state() {
  ...
  jq -nc --arg t "$(date --iso-8601=seconds 2>/dev/null || date -Iseconds)" \
        --arg phase "$phase" --arg kind "$kind" --arg ns "$ns" --arg name "$name" \
        --arg extra "$extra" \
        '{ts:$t, phase:$phase, kind:$kind, namespace:$ns, name:$name, extra:$extra}' \
        >> "${STATE_FILE}"
}
...
is_done() {
  ...
  jq -r --arg k "$kind" --arg n "$ns" --arg m "$name" \
     'select(.kind==$k and .namespace==$n and .name==$m and .phase=="verified") | 1' \
     "${STATE_FILE}" | grep -q 1
}
...
apply_all() {
  ...
  while read -r item; do
    ...
    if is_done "$kind" "$ns" "$name"; then
      echo "Atlanıyor (verified): ${kind}/${name} (${ns})"; continue; fi
...
interactive() {
  # prompt_read TTY üzerinden okur; stdin pipe olsa bile etkilenmez.
  local all; all="$(fetch_workloads)"
